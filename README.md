...
- Supports automatic download of user defined binaries and upload them to the degault Filer path
...
Installs Docker runtime, SeaweedFS container, and Caddy container from the internet, unless a valid ```swfs-save.tar.gz``` is in the same directory as ```install-seaweedfs``` or ```-registry``` is used to pull from a local registry. SeaweedFS configuration is located in ```/opt/seaweedfs```, including Filer data. Caddy configuration is located in ```/opt/caddy```.
...
First pulls containers from the internet or loads them locally from ```swfs-save.tar.gz```, then pushes the containers to the specified registry. Note that the registry must have the ```/chrislufs``` and ```/library``` projects pre-created in order to push the containers.

### Registry

When specified, will attempt to obtain the registries TLS certificate and login via docker in order to push and/or pull containers

# Custom Variables

Edit the ```install-seaweedfs``` file and edit the ```USER DEFINED VARIABLES``` section to modify the default behavior.  
...
|SWFS_IMAGE            |chrislusf/seaweedfs:latest |Project, image, and tag for SeaweedFS container |
|CADDY_IMAGE           |caddy:latest               |Project, image, and tag for Caddy container     |
...
|ARTIFACTS_TO_DOWNLOAD |""                         |List of binary URLs for filer upload. I.e "https://static/file1.txt" "https://static/image1.img" |